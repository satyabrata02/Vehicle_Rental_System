...
def auth_verify(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            print(e)
    return render(request, 'authentication.html', {"check":check, "login_type":login_type})
...
def reg_auth(request):
    ...
    if request.method == 'GET':
        try:
            ...
            has_auth = user.is_set_auth
            if not has_auth:
                key = pyotp.random_base32()
                request.session['auth_key'] = key
                
                uri = pyotp.totp.TOTP(key).provisioning_uri(name=str(user.phoneno), issuer_name="Vehicle Rent")
                qr = qrcode.make(uri)
                buffer = io.BytesIO()
            ...
            print(e)
            has_auth = False
        return render(request, 'add_auth.html', {"qr_code":qr_code, "has_auth":has_auth, 'user':user, 'login_type':login_type})
...
def turnoff_auth(request):
    ...
    return render(request, 'turnoff_auth.html', {"phoneno":phoneno, "login_type":login_type})
